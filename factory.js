...
(function () {
  ...
  var defaultSettings = {
    axisHelper: 0, //show an axis helper in the scene
  ...
  var constructor = {
    ...
    renderer: {
      ...
      _intro: function (options) {
        include(this, options, {
          width: 500, height: 500, reuse: true, container: undefined
        });
        if (jQuery && THREE) {
          if (this.reuse) {
...
//build a json based on objects
  ...
  function loadScene(json, settings) {
    //if json is not an object, it an url (?)
    json = (typeof json == 'object') ? json : require(json);
    //if no settings or scene present, make one
    if (!_.size(json.settings)) updatePack(json, 'settings', defaultSettings);
    if (!_.size(json.scene)) updatePack(json, 'scene');
    //override settings
    settings = _.extend(_.find(json.settings, _.identity), settings);
    ...
    module.exports.loadObjects(json); //for worker as well
